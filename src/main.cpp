...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() != CBaseChainParams::MAIN)
        return nHeight >= Params().TreasuryStartBlock() ? 85 * COIN : 100 * COIN;
    ...
    if (nHeight < 200)
        ...
        nSubsidy = 50 * COIN;
    else if (nHeight > 500000 && nHeight <= 590000) // currently here
        nSubsidy = 60 * COIN;
    // NEW REWARDS: Start new after ~ Thursday, December 19, 2019 and protocol 70940
    else if (nHeight > 590000 && nHeight <= 700000)
        nSubsidy = 7 * COIN;
        ...
        nSubsidy = 2 * COIN; // OPTIONAL: Halving can take over from here when NOT 0
    ...
    // treasury value adjustment
    if (nHeight >= Params().TreasuryStartBlock())
        nSubsidy = nSubsidy * 8.5 / 10; // remove 15% for treasury (85/100 goes to stakers and MNs)
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    // No rewards until masternode activation.
    if (nHeight <= Params().LAST_POW_BLOCK() || blockValue == 0)
        return 0;

    // treasury value adjustment
    if (nHeight >= Params().TreasuryStartBlock())
        blockValue = blockValue * 10 / 8.5; // add back treasury percentage to get original block value
    ...
    else if (nHeight > 590000 && nHeight <= 600000) // time before dev fund starts
        ret = blockValue * 0.70; // 70% of block reward
    else
        // set static percentage going forward...
...
{
    // if( chainActive.Height() >= 590000 )
    //     return value == MASTERNODE_COLLATERAL_AFTER_590000;
...
{
    // if( chainActive.Height() >= 590000 )
    //     return MASTERNODE_COLLATERAL_AFTER_590000;
    ...
    if (IsTreasuryBlock(nHeight)) {
        ...
        for (int i = startHeight; i < nHeight; i++) {
            if (!IsTreasuryBlock(i)) // make sure previous treasury rewards aren't counted
                ...
                blockValue += GetBlockValue(i);
            }
        }
        
        blockValue = blockValue * 10 / 8.5; // add back treasury payment to get original block value
        
        return blockValue / 10; // return block value paid to treasury
    } else {
        return 0;
    }
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    nExpectedMint += GetBlockValue( (pindex->nHeight > 225000 ? pindex->nHeight : pindex->pprev->nHeight) );
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
